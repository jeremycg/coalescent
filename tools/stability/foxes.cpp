// Standalone stability + calibration suite for Foxes' three-species food-chain
// kernel. Calls the SDK-free production Hastings–Powell core and independently
...
#include <algorithm>
#include <vector>
#include "../../src/dsp/foxes_core.hpp"

namespace model = coalescent::foxes;
...
static void jacobian(double x, double y, double z, double b1, double b2, double J[3][3]) {
    double g1 = model::f1(x, b1), g2 = model::f2(y, b2);
    double g1p = model::A1 / ((1 + b1 * x) * (1 + b1 * x));
    double g2p = model::A2 / ((1 + b2 * y) * (1 + b2 * y));
    J[0][0] = 1 - 2 * x - g1p * y; J[0][1] = -g1;                 J[0][2] = 0;
    J[1][0] = g1p * y;             J[1][1] = g1 - g2p * z - model::D1;   J[1][2] = -g2;
    J[2][0] = 0;                   J[2][1] = g2p * z;             J[2][2] = g2 - model::D2;
}

struct Diag { float amp = 0, period = 0, minState = 0, maxState = 0; float floorFrac = 0, ceilFrac = 0; };

// Float production-core driver: adaptive substep, floor/ceiling backstop after
...
static bool run(double bal, double wild, float kick, float dtau, int NS, Diag& dg) {
    float b1 = model::b1FromWild((float) wild);
    float b2 = model::b2FromBalance((float) bal);
    double xs, ys, zs;
    if (!model::equilibrium(model::b1FromWild(wild), model::b2FromBalance(bal), xs, ys, zs)) return true;
    const coalescent::OdeStepPlan plan = model::stepPlan(dtau);
    float kf = kick * model::KICK_GAIN;
    ...
    for (int s = 0; s < NS; s++) {
        for (int k = 0; k < plan.count; k++) {
            model::step(y, plan.h, b1, b2, kf);
            for (int j = 0; j < 3; j++) {
                if (!std::isfinite(y[j])) return false;           // check BEFORE clamp: don't let it mask NaN
                if (y[j] <= model::POS_FLOOR * 1.5f) floorHits++;
                if (y[j] >= model::STATE_MAX * 0.99f) ceilHits++;
                tot++;
            }
        ...
        if (s > NS / 2) {
            ...
            if (up) { float frac = cc / (cc - prev), t = (s - frac) * plan.delta;
...
static double lyapunov(double b1, double b2, double h, double burn, double span) {
    double xs, ys, zs;
    if (!model::equilibrium(b1, b2, xs, ys, zs)) return NAN;
    double y[3] = {xs * 1.2, ys, zs};
    for (double t = 0; t < burn; t += h) model::step(y, h, b1, b2, 0.0);

    // coupled derivative of s = [state(0..2), tangent(3..5)]
    auto fc = [&](const double s[6], double o[6]) {
        model::derivative(s, o, b1, b2, 0.0);
...
static void runFromSeed(double bal, double wild, float kick,
    ...
    float b1 = model::b1FromWild((float) wild);
    float b2 = model::b2FromBalance((float) bal);
    float cen[3]; model::equilibrium(b1, b2, cen[0], cen[1], cen[2]);
    const coalescent::OdeStepPlan plan = model::stepPlan(0.025f);
    float y[3];
    model::seed(y);
    auto advance = [&]() {
        for (int k = 0; k < plan.count; k++) {
            model::step(y, plan.h, b1, b2, kick);
            model::repairState(y);
        }
    };
    for (int s = 0; s < 160000; s++) advance();              // burn in past the transient (~4000 tau)
    double sum[3] = {0, 0, 0}, lo[3] = {9, 9, 9}, hi[3] = {-9, -9, -9}; long railed[3] = {0, 0, 0}, n = 0;
    for (int s = 0; s < 100000; s++) { advance();
        for (int i = 0; i < 3; i++) {
            double v = model::outputVoltage(y[i] - cen[i], i);
...
// Peak-gate contract from the production seed. Drives the shared rising-to-falling
// detector at the production per-substep observation point.
static void gateCounts(double bal, double wild, int counts[3]) {
    float b1 = model::b1FromWild((float) wild);
    float b2 = model::b2FromBalance((float) bal);
    float cen[3]; model::equilibrium(b1, b2, cen[0], cen[1], cen[2]);
    const coalescent::OdeStepPlan plan = model::stepPlan(0.02f);
    float y[3];
    model::seed(y);
    auto advance = [&]() {
        for (int k = 0; k < plan.count; k++) {
            model::step(y, plan.h, b1, b2, 0.f);
            model::repairState(y);
        }
    };
    float prev[3] = {0, 0, 0}; bool rising[3] = {false, false, false};
    counts[0] = counts[1] = counts[2] = 0;
    for (int s = 0; s < 160000; s++) advance();              // burn in past the transient
    for (int s = 0; s < 40000; s++) {
        for (int k = 0; k < plan.count; k++) {
            model::step(y, plan.h, b1, b2, 0.f);
            model::repairState(y);
            for (int i = 0; i < 3; i++) {
                float gained = (y[i] - cen[i]) * model::gain(i);
                if (model::peakStep(gained, prev[i], rising[i])) counts[i]++;
            }
        }
    }
}
...
static double gateOrder() {
    double b1 = model::b1FromWild(0.5), b2 = 2.0, xs, ys, zs;
    model::equilibrium(b1, b2, xs, ys, zs);
    double cen[3] = {xs, ys, zs}, y[3] = {xs * 1.2, ys, zs}, h = 0.01;
    for (double t = 0; t < 5000; t += h) model::step(y, h, b1, b2, 0.0);
    std::vector<double> pk[3]; double prev[3] = {0, 0, 0}; bool ris[3] = {false, false, false};
    double tau = 0;
    for (double t = 0; t < 3000; t += h) {
        model::step(y, h, b1, b2, 0.0); tau += h;
        for (int i = 0; i < 3; i++) {
            double gained = (y[i] - cen[i]) * model::gain(i);
            if (model::peakStep(gained, prev[i], ris[i])) pk[i].push_back(tau);
        }
    }
    int ok = 0, tot = 0;
...
static double subharmonicDb() {
    ...
    for (double t = 0; t < 5000; t += h) model::step(y, h, b1, b2, 0.0);
    double P = model::RATE_CAL, f1 = 1.0 / P, f2 = 1.0 / (2 * P);
    double re1 = 0, im1 = 0, re2 = 0, im2 = 0, tau = 0; long N = 0;
    while (tau < 20 * P) {
        model::step(y, h, b1, b2, 0.0); double g = y[0] - xs;
...
static void foxMaxSpread(double b1, double b2, double h, double& lo, double& hi, int& count) {
    double xs, ys, zs; model::equilibrium(b1, b2, xs, ys, zs);
    double y[3] = {xs * 1.2, ys, zs};
    for (double t = 0; t < 4000; t += h) model::step(y, h, b1, b2, 0.0);
    std::vector<double> mx; double zprev = y[2]; bool up = false;
    for (double t = 0; t < 12000; t += h) {
        model::step(y, h, b1, b2, 0.0);
...
int main() {
    ...
    for (double bal = 0; bal <= 1.0001; bal += 0.05)
        for (double wild = 0; wild <= 1.0001; wild += 0.05) {
            double b1 = model::b1FromWild(wild), b2 = model::b2FromBalance(bal), xs, ys, zs;
            if (!model::equilibrium(b1, b2, xs, ys, zs)) { invalid++; continue; }
            double s[3] = {xs, ys, zs}, d[3]; model::derivative(s, d, b1, b2, 0.0);
    ...
    for (double b1 = 1.0; b1 <= 3.0; b1 += 0.0005) {
        double b2 = 2.0, xs, ys, zs; if (!model::equilibrium(b1, b2, xs, ys, zs)) continue;
    ...
    { auto amp = [](double b1, double b2, double sx, double sy, double sz) {
          double xs, ys, zs;
          if (!model::equilibrium(b1, b2, xs, ys, zs)) return 0.0;
          double y[3] = {xs * sx, ys * sy, zs * sz};
          for (double t = 0; t < 6000; t += 0.01) model::step(y, 0.01, b1, b2, 0.0);
          double mn = 1e9, mx = -1e9;
          for (double t = 0; t < 600; t += 0.01) { model::step(y, 0.01, b1, b2, 0.0); mn = std::min(mn, y[0]); mx = std::max(mx, y[0]); }
          return mx - mn;
      };
      int multi = 0;
    ...
    { double b1 = model::b1FromWild(0.5), b2 = 2.0, xs, ys, zs; model::equilibrium(b1, b2, xs, ys, zs);
      double y[3] = {xs * 1.2, ys, zs};
      for (double t = 0; t < 5000; t += 0.01) model::step(y, 0.01, b1, b2, 0.0);
    ...
    // ── [8] shared production input-sanitization policy ──
    ...
    { bool ok = true;
      for (float bad : {(float) NAN, (float) INFINITY, -(float) INFINITY, 1e30f, -1e30f}) {
          float p = coalescent::finiteClamp(bad, 0.f, -8.f, 8.f);
          float k = coalescent::finiteClamp(bad, 0.f, -15.f, 15.f) * model::KICK_GAIN;
          if (!std::isfinite(p) || p < -8.f || p > 8.f) ok = false;
          if (!std::isfinite(k) || std::fabs(k) > 15.f * model::KICK_GAIN + 1e-3f) ok = false;
      ...
      if (coalescent::finiteClamp((float) NAN, 0.f, -8.f, 8.f) != 0.f) ok = false;
    ...
    { Diag dc; run(0.5, 0.5, 0.f, model::HSUB_MAX * 1.999f, 200000, dc);   // adaptive h → HSUB_MAX
      double pCoarse = dc.period;
      double b1 = model::b1FromWild(0.5), b2 = 2.0, xs, ys, zs; model::equilibrium(b1, b2, xs, ys, zs);
      double y[3] = {xs * 1.2, ys, zs};
      for (double t = 0; t < 4000; t += 0.005) model::step(y, 0.005, b1, b2, 0.0);
      double prev = 0, lc = -1, sp = 0, T = 0; int np = 0;
      for (double t = 0; t < 8000; t += 0.005) {
          model::step(y, 0.005, b1, b2, 0.0); T += 0.005; double cc = y[0] - xs;
    ...
    { auto maxSpread = [](double b1, double b2) {
          double xs, ys, zs; model::equilibrium(b1, b2, xs, ys, zs); double y[3] = {xs * 1.2, ys, zs};
          for (double t = 0; t < 5000; t += 0.005) model::step(y, 0.005, b1, b2, 0.0);
          std::vector<double> m; double zp = y[2]; bool up = false;
          for (double t = 0; t < 3000; t += 0.005) {
              model::step(y, 0.005, b1, b2, 0.0);
      ...
      auto dRes = maxSpread(model::b1FromWild(0.5), 2.0);
      auto tRes = maxSpread(model::b1FromWild(0.57), 2.0);
      double subDb = subharmonicDb();
      double lyTrans = lyapunov(model::b1FromWild(0.57), 2.0, 0.01, 3000, 6000);
    ...
    { double y[3] = {model::SEED_X, model::SEED_Y, model::SEED_Z};
      double ranges[2][3] = {};
      for (int window = 0; window < 2; window++) {
          double lo[3] = {1e9, 1e9, 1e9}, hi[3] = {-1e9, -1e9, -1e9};
          for (int p = 0; p < model::TRAIL_N; p++) {
              // CAP_TAU / 9 is exact enough here and follows the portrait's uniform
              // dimensionless-time spacing without involving audio sample rate.
              for (int k = 0; k < 9; k++)
                  model::step(y, static_cast<double>(model::CAP_TAU) / 9.0,
                              model::b1FromWild(0.0), 2.0, 0.0);
